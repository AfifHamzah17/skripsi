...
function Field({ label, icon: Icon, value, isEditing, onChange, type = 'text', disabled = false, href, children }) {
  const content = href && !isEditing ? (<a href={href} target={href.startsWith('http') ? '_blank' : undefined} rel={href.startsWith('http') ? 'noopener noreferrer' : undefined} style={{ fontSize: 14, color: '#2563eb', fontWeight: 500, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 6 }}>{value || <span style={{ color: '#d1d5db', fontWeight: 400 }}>—</span>}</a>) : (value || <span style={{ color: '#d1d5db', fontWeight: 400 }}>—</span>);
  return (
    <div style={{ display: 'flex', alignItems: 'center', padding: '14px 0', borderBottom: '1px solid #f3f4f6', gap: 16 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {children || (isEditing && !disabled ? (<input type={type} value={value || ''} onChange={e => onChange(e.target.value)} style={{ width: '100%', padding: '10px 14px', border: '1px solid #d1d5db', borderRadius: 8, fontSize: 14, outline: 'none', boxSizing: 'border-box', transition: 'border 0.15s' }} onFocus={e => e.target.style.borderColor = '#2563eb'} onBlur={e => e.target.style.borderColor = '#d1d5db'} placeholder={`Masukkan ${label.toLowerCase()}`} />) : (<p style={{ fontSize: 14, color: '#111827', fontWeight: 500, margin: 0 }}>{content}</p>))}
      </div>
    </div>
  );
}

export default function ProfileView({ user, loading, error, isOwnProfile, isEditing, onEditToggle, onSave, onCancel, editData, onEditChange, avatarUploading, onAvatarSelect, cropModal, onCropChange, onZoomChange, onCropComplete, onCropConfirm, onCropCancel, statistics, rosterMapel }) {
    ...
    <div style={{ minHeight: '100vh', background: '#f9fafb' }}>
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '32px 16px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.06)', border: '1px solid #f3f4f6', overflow: 'hidden' }}>
          <div style={{ height: 144, background: 'linear-gradient(135deg,#2563eb,#4f46e5,#7c3aed)', position: 'relative', overflow: 'hidden' }}><div style={{ position: 'absolute', inset: 0, opacity: 0.1 }}><svg width="100%" height="100%" viewBox="0 0 400 150"><circle cx="50" cy="30" r="80" fill="white" /><circle cx="350" cy="120" r="60" fill="white" /><circle cx="200" cy="-20" r="100" fill="white" /></svg></div></div>
          <div style={{ padding: '0 24px 24px', marginTop: -56, position: 'relative' }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', gap: 16 }}>
              <div style={{ position: 'relative', borderRadius: 16, overflow: 'hidden', boxShadow: '0 4px 12px rgba(0,0,0,0.15)', border: '4px solid #fff', cursor: isOwnProfile ? 'pointer' : 'default', width: 112, height: 112, flexShrink: 0 }} onClick={() => isOwnProfile && fileRef.current?.click()}>
                ...
                {isOwnProfile && <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 12, opacity: avatarUploading ? 1 : 0, transition: 'opacity 0.2s' }}>{avatarUploading ? <div style={{ width: 24, height: 24, borderRadius: '50%', border: '2px solid #fff', borderTopColor: 'transparent', animation: 'spin 1s linear infinite' }} /> : <div style={{ textAlign: 'center', color: '#fff' }}><FaCamera style={{ fontSize: 18, display: 'block', marginBottom: 2 }} /><span style={{ fontSize: 9, fontWeight: 500 }}>Ubah Foto</span></div>}</div>}
              ...
              <div style={{ flex: 1, minWidth: 0, paddingTop: 4 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}><span style={{ padding: '3px 10px', fontSize: 11, fontWeight: 700, borderRadius: 9999, border: `1px solid ${c[2]}`, background: c[0], color: c[1], textTransform: 'uppercase', letterSpacing: '0.03em' }}>{user.role}</span>{user.verified && <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: '#3b82f6', fontWeight: 500 }}><FaShieldAlt />Terverifikasi</span>}</div>
              </div>
              <div style={{ display: 'flex', gap: 10, flexShrink: 0 }}>
                {user.role === 'guru' && !isEditing && <AdminBtn />}
                {isOwnProfile && (<button onClick={isEditing ? onCancel : onEditToggle} style={{ padding: '10px 20px', borderRadius: 10, fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', border: isEditing ? '1px solid #d1d5db' : 'none', background: isEditing ? '#f9fafb' : '#2563eb', color: isEditing ? '#4b5563' : '#fff', boxShadow: isEditing ? 'none' : '0 2px 8px rgba(37,99,235,0.3)', transition: 'all 0.15s' }} onMouseEnter={e => { if (!isEditing) e.currentTarget.style.background = '#1d4ed8'; }} onMouseLeave={e => { if (!isEditing) e.currentTarget.style.background = '#2563eb'; }}>{isEditing ? <><FaTimes style={{ fontSize: 12 }} />Batal</> : <><FaEdit style={{ fontSize: 12 }} />Edit Profil</>}</button>)}
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.06)', border: isEditing ? '2px solid #2563eb' : '1px solid #f3f4f6', overflow: 'hidden', transition: 'border 0.2s' }}>
          <div style={{ padding: '16px 24px', borderBottom: '1px solid #f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}><h2 style={{ fontSize: 15, fontWeight: 700, color: '#111827', margin: 0 }}>Informasi Pribadi</h2>{isEditing && <span style={{ fontSize: 11, color: '#fff', fontWeight: 700, background: '#2563eb', padding: '4px 12px', borderRadius: 9999 }}>MODE EDIT</span>}</div>
          <div style={{ padding: '0 24px' }}>
            ...
            {user.role === 'guru' && (<>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', padding: '14px 0', borderBottom: '1px solid #f3f4f6', gap: 16 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  {rosterMapel?.length > 0 ? (<div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>{rosterMapel.map(m => (<div key={m.nama} style={{ padding: '8px 12px', background: '#faf5ff', borderRadius: 8, border: '1px solid #e9d5ff' }}><div style={{ fontWeight: 600, color: '#6d28d9', fontSize: 13, marginBottom: 4 }}>{m.nama}</div><div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>{m.kelasList.map(k => <span key={k.kelas} style={{ fontSize: 11, padding: '2px 8px', background: '#fff', color: '#7c3aed', borderRadius: 9999, border: '1px solid #ddd6fe', fontWeight: 500 }}>{k.kelas}</span>)}</div></div>))}</div>) : (<div style={{ padding: '10px 12px', background: '#f9fafb', borderRadius: 8, border: '1px dashed #e5e7eb' }}><span style={{ fontSize: 13, color: '#9ca3af' }}>Belum ada jadwal mengajar</span></div>)}
                  {isEditing && <p style={{ fontSize: 11, color: '#9ca3af', margin: '6px 0 0' }}>Mapel diatur otomatis dari roster jadwal</p>}
                </div>
              </div>
            </>)}
          ...
          {isEditing && (<div style={{ padding: '20px 24px', borderTop: '1px solid #f3f4f6', display: 'flex', justifyContent: 'flex-end' }}><button onClick={onSave} style={{ padding: '12px 32px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, boxShadow: '0 4px 12px rgba(37,99,235,0.3)', transition: 'background 0.15s' }} onMouseEnter={e => e.currentTarget.style.background = '#1d4ed8'} onMouseLeave={e => e.currentTarget.style.background = '#2563eb'}><FaSave style={{ fontSize: 13 }} />Simpan Perubahan</button></div>)}
        </div>
      ...
      {cropModal.isOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          ...
          <div style={{ position: 'relative', background: '#fff', borderRadius: 16, boxShadow: '0 25px 50px rgba(0,0,0,0.2)', width: '100%', maxWidth: 400, overflow: 'hidden' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}><h3 style={{ fontSize: 15, fontWeight: 700, color: '#111827', margin: 0 }}>Potong Foto</h3><button type="button" onClick={onCropCancel} style={{ padding: 6, background: '#f3f4f6', border: 'none', borderRadius: 6, cursor: 'pointer', color: '#6b7280' }}><FaTimes style={{ fontSize: 12 }} /></button></div>
