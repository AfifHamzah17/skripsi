...
import { FaUser, FaLock, FaEye, FaEyeSlash, FaIdCard, FaEnvelope, FaPhone, FaUserFriends, FaChevronDown, FaSpinner } from 'react-icons/fa';
...
export default function UserForm({ initialData = null, onSubmit, onCancel, loading = false }) {
  const [form, setForm] = useState({ nama: '', password: '', role: 'siswa', email: '', nohp: '', nisn: '', nip: '', posisi: '', tingkat: '', jurusan: '', nomor: '', kelas: '' });
  ...
  useEffect(() => {
    if (initialData) {
      ...
      const parts = k.split(' ');
      setForm({ nama: initialData.nama || '', password: '', role: initialData.role || 'siswa', email: initialData.email || '', nohp: initialData.nohp ? fmtPhone(initialData.nohp) : '', nisn: initialData.nisn || '', nip: initialData.nip || '', posisi: initialData.posisi || '', tingkat: parts[0] || '', jurusan: parts[1] || '', nomor: parts[2] || '', kelas: k });
    }
  }, [initialData]);
  ...
  const handleSubmit = ev => {
    ...
    if (form.role !== 'guru') delete data.nip;
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <Grp><Label req>Nama Lengkap</Label><Wrap><Icon><FaUser /></Icon><input type="text" value={form.nama} onChange={e => set('nama', e.target.value)} onFocus={() => setFocus('nama')} onBlur={() => setFocus(null)} style={iS('nama')} placeholder="Nama lengkap" /></Wrap><Err msg={errors.nama} /></Grp>
      {!initialData && (<Grp><Label req>Password</Label><Wrap><Icon><FaLock /></Icon><input type={showP ? 'text' : 'password'} value={form.password} onChange={e => set('password', e.target.value)} onFocus={() => setFocus('pw')} onBlur={() => setFocus(null)} style={{ ...iS('pw'), paddingRight: '2.25rem' }} placeholder="Min. 8 karakter, huruf besar & angka/simbol" /><EyeBtn show={showP} onToggle={() => setShowP(!showP)} /></Wrap><PwDot pw={form.password} /><Err msg={errors.password} /></Grp>)}
      <Grp><Label req>Role</Label><Wrap><Icon><FaUserFriends /></Icon><select value={form.role} onChange={e => { set('role', e.target.value); setErrors(p => ({ ...p, kelas: null, nisn: null, nip: null, posisi: null })); }} onFocus={() => setFocus('role')} onBlur={() => setFocus(null)} style={{ ...iS('role'), appearance: 'none', paddingRight: '2rem', cursor: 'pointer' }}><option value="siswa">Siswa</option><option value="guru">Guru</option><option value="petugas">Petugas</option></select><FaChevronDown style={{ position: 'absolute', top: '50%', right: '0.75rem', transform: 'translateY(-50%)', color: '#9ca3af', fontSize: '0.75rem', pointerEvents: 'none' }} /></Wrap></Grp>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '0.75rem' }}>
        <Grp><Label>Email</Label><Wrap><Icon><FaEnvelope /></Icon><input type="email" value={form.email} onChange={e => set('email', e.target.value)} onFocus={() => setFocus('em')} onBlur={() => setFocus(null)} style={iS('em')} placeholder="contoh@email.com" /></Wrap></Grp>
        <Grp><Label>No. HP <span style={{ fontSize: '0.625rem', color: '#9ca3af', fontWeight: 400 }}>(format otomatis)</span></Label><Wrap><Icon><FaPhone /></Icon><input type="text" value={form.nohp} onChange={e => set('nohp', fmtPhone(e.target.value))} onFocus={() => setFocus('hp')} onBlur={() => setFocus(null)} style={iS('hp')} placeholder="0812-3456-7890" inputMode="numeric" /></Wrap><Err msg={errors.nohp} />{form.nohp && <p style={{ fontSize: '0.625rem', color: '#9ca3af', margin: '2px 0 0' }}>{parsePhone(form.nohp).length} digit</p>}</Grp>
      </div>
      {form.role === 'siswa' && (<>
        <div style={{ marginBottom: '0.75rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.5rem' }}>
            <div><label style={{ fontSize: '0.625rem', color: '#6b7280', marginBottom: '0.25rem', display: 'block' }}>Tingkat</label><select value={form.tingkat} onChange={e => set('tingkat', e.target.value)} style={{ width: '100%', paddingLeft: '0.5rem', appearance: 'none', backgroundColor: '#fff', fontSize: '0.75rem', paddingTop: '0.375rem', paddingBottom: '0.375rem', borderWidth: '1px', borderStyle: 'solid', borderColor: errors.kelas ? '#ef4444' : '#d1d5db', borderRadius: '0.5rem', outline: 'none', transition: 'box-shadow 200ms, border-color 200ms', boxSizing: 'border-box', ...(errors.kelas ? { boxShadow: '0 0 0 3px rgba(239,68,68,0.1)' } : {}) }}><option value="">-- Pilih --</option>{TINGKAT.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
            <div><label style={{ fontSize: '0.625rem', color: '#6b7280', marginBottom: '0.25rem', display: 'block' }}>Jurusan</label><select value={form.jurusan} onChange={e => set('jurusan', e.target.value)} style={{ width: '100%', paddingLeft: '0.5rem', appearance: 'none', backgroundColor: '#fff', fontSize: '0.75rem', paddingTop: '0.375rem', paddingBottom: '0.375rem', borderWidth: '1px', borderStyle: 'solid', borderColor: errors.kelas ? '#ef4444' : '#d1d5db', borderRadius: '0.5rem', outline: 'none', transition: 'box-shadow 200ms, border-color 200ms', boxSizing: 'border-box', ...(errors.kelas ? { boxShadow: '0 0 0 3px rgba(239,68,68,0.1)' } : {}) }}><option value="">-- Pilih --</option>{JURUSAN.map(j => <option key={j.v} value={j.v}>{j.l}</option>)}</select></div>
            <div><label style={{ fontSize: '0.625rem', color: '#6b7280', marginBottom: '0.25rem', display: 'block' }}>Nomor</label><select value={form.nomor} onChange={e => set('nomor', e.target.value)} style={{ width: '100%', paddingLeft: '0.5rem', appearance: 'none', backgroundColor: '#fff', fontSize: '0.75rem', paddingTop: '0.375rem', paddingBottom: '0.375rem', borderWidth: '1px', borderStyle: 'solid', borderColor: errors.kelas ? '#ef4444' : '#d1d5db', borderRadius: '0.5rem', outline: 'none', transition: 'box-shadow 200ms, border-color 200ms', boxSizing: 'border-box', ...(errors.kelas ? { boxShadow: '0 0 0 3px rgba(239,68,68,0.1)' } : {}) }}><option value="">-- Pilih --</option>{NOMOR.map(n => <option key={n} value={n}>{n}</option>)}</select></div>
          </div>
        ...
        <Grp><Label req>NISN</Label><Wrap><Icon><FaIdCard /></Icon><input type="text" value={form.nisn} onChange={e => set('nisn', e.target.value)} maxLength={10} onFocus={() => setFocus('nisn')} onBlur={() => setFocus(null)} style={iS('nisn')} placeholder="10 digit NISN" /></Wrap><Err msg={errors.nisn} /></Grp>
      </>)}
      {form.role === 'guru' && (<Grp><Label>NIP <span style={{ fontSize: '0.625rem', color: '#9ca3af', fontWeight: 400 }}>(opsional)</span></Label><Wrap><Icon><FaIdCard /></Icon><input type="text" value={form.nip} onChange={e => set('nip', e.target.value)} onFocus={() => setFocus('nip')} onBlur={() => setFocus(null)} style={iS('nip')} placeholder="Nomor Induk Pegawai" /></Wrap></Grp>)}
      {form.role === 'petugas' && (<Grp><Label req>Posisi</Label><Wrap><Icon><FaUserFriends /></Icon><input type="text" value={form.posisi} onChange={e => set('posisi', e.target.value)} onFocus={() => setFocus('pos')} onBlur={() => setFocus(null)} style={iS('pos')} placeholder="Contoh: Lab Komputer" /></Wrap><Err msg={errors.posisi} /></Grp>)}
